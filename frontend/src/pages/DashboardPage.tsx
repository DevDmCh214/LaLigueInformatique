...
export default function DashboardPage() {
  ...
  const filteredEvenements = data.evenements.filter((ev: any) => sportIds.has(ev.sport?.id ?? ev.sportId));
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="card text-center">
          ...
          <p className="text-3xl font-bold text-gray-600 mt-1">{filteredEvenements.length}</p>
        </div>
        <div className="card text-center">
      ...
      <div className="mb-6">
        ...
        {data.equipes.length === 0 ? (
          <p className="text-gray-400 text-sm">Aucune equipe. Rejoignez une equipe !</p>
        ) : (
            ...
            {data.equipes.map((eq: any) => (
              <Link key={eq.id} to={`/equipes/${eq.id}`} className="card hover:shadow-md hover:border-gray-400 transition-all">
                <div className="flex justify-between items-start">
                  ...
                  {eq.sport && <span className="badge-blue">{eq.sport.nom}</span>}
                </div>
