...
export default function NewMatchPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    router.push(`/matchs/${ev.match.id}`);
  }
      ...
      <div className="card max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              value={form.sportId}
              onChange={(e) => setForm({ ...form, sportId: Number(e.target.value) })}
              required
              className="input"
            >
              {sports.map((s) => (
          ...
          <div>
            ...
            <input
              type="text"
              value={form.entitule}
              onChange={(e) => setForm({ ...form, entitule: e.target.value })}
              required
              className="input"
              placeholder="Ex: FC Paris vs OGC Nice"
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                value={form.equipe1Id}
                onChange={(e) => setForm({ ...form, equipe1Id: Number(e.target.value) })}
                required
                className="input"
              >
                {equipes.map((eq) => (
            ...
            <div>
              ...
              <select
                value={form.equipe2Id}
                onChange={(e) => setForm({ ...form, equipe2Id: Number(e.target.value) })}
                required
                className="input"
              >
                {equipes.map((eq) => (
