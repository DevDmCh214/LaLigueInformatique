...
async function main() {
  ...
  // Equipes
  const fcParis = await prisma.equipe.create({ data: { nom: "FC Paris", nombrePlaces: 11 } });
  const lyonBasket = await prisma.equipe.create({ data: { nom: "Lyon Basket", nombrePlaces: 5 } });
  const marseilleVolley = await prisma.equipe.create({ data: { nom: "Marseille Volley", nombrePlaces: 6 } });
  const niceFC = await prisma.equipe.create({ data: { nom: "OGC Nice", nombrePlaces: 11 } });
  ...
  const match3 = await prisma.match.create({ data: { evenementId: evMatch3.id, equipeGagnanteId: fcParis.id } });
  await prisma.equipeParticipante.createMany({
    data: [
